...
public class CrafterBlockEntity extends InventoryBlockEntity {
    public CrafterBlockEntity(CompoundTag nbt) {
        super(nbt);
    }

    public int getCraftingTicksRemaining() {
        return getInt("crafting_ticks_remaining");
    }

    public boolean isTriggered() {
        return getByte("triggered") == 1;
    }

    public int[] getDisabledSlots() {
        return getNbt().getIntArray("disabled_slots", new int[]{0});
    }
    ...
    public long getLootTableSeed() {
        return getNbt().getLong("LootTableSeed", 0L);
    }
}
